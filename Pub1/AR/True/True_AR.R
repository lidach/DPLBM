...
# setwd("D:/DPLBM/Pub1/AR")
# AR <- readRDS("D:/DPLBM/Pub1/AR/LFQARmodel.rds")
AR <- readRDS("LFQARmodel.rds")
list <- readRDS("ARmodel_list.rds")
iters <- 300
...
setwd("D:/DPLBM/Pub1/AR/True")
saveRDS(AR1, file = "LFQARmodel.rds")
...
res_cc <- list()
for (i in 1:iters){
  res_cc[[i]] <- catchCurve(LFQARmodel2[[i]], reg_int = c(7,28), calc_ogive = T)
}


# res_cc[[i]] <- catchCurve(LFQARmodel2[[i]], reg_int = NULL, calc_ogive = T)
...
for(i in 1:iters){
  LFQARmodel2[[i]]$Z <- res_cc[[i]]$Z
  LFQARmodel2[[i]]$FM <- as.numeric(res_cc[[i]]$Z - res_cc[[i]]$M)
  LFQARmodel2[[i]]$E <- res_cc[[i]]$FM[[1]]/res_cc[[i]]$Z
  LFQARmodel2[[i]]$L50 <- res_cc[[i]]$L50
  LFQARmodel2[[i]]$L75 <- res_cc[[i]]$L75
}

saveRDS(res_cc, file = "AR_res_cc.rds")


#' CA
LFQARmodel3 <- LFQARmodel2
# a <- c(which(p %in% NA))
source("D:/Thesis/R/lfqModifydev.R")
for(i in 1:iters){
  LFQARmodel3[[i]] <- lfqModifydev(LFQARmodel3[[i]], plus_group = TRUE)
...
vpa_res <- list()
for(i in 1:iters){
  vpa_res[[i]] <- VPA(LFQARmodel3[[i]], terminalF = LFQARmodel3[[i]]$FM, analysis_type = "CA", plot = F)
}

for(i in 1:iters){
  LFQARmodel3[[i]]$FM <- vpa_res[[i]]$FM_calc
}
...
saveRDS(vpa_res, file = "AR_res_CA.rds")
...
#' save data
saveRDS(res_TB2, file = "AR_res_TB2.rds")
...
for(i in 1:iters){
  ...
  K[[i]] <- LFQARmodel1[[i]]$K
  M[[i]] <- LFQARmodel1[[i]]$M
  Lmat[[i]] <- LFQARmodel1[[i]]$Lmat
  midLengths[[i]] <- LFQARmodel1[[i]]$midLengths
}

AR_res_LBRP <- list()
for(i in 1:iters){
  AR_res_LBRP[[i]] <- Froese.plus(catch_prop[[i]], midLengths[[i]], Linf[[i]], K[[i]], t0, M[[i]], a, b, Lmat[[i]], ages[[i]])
}

#' save data
saveRDS(AR_res_LBRP, file = "ARmodel_res_LBRP.rds")
...
setwd(modpath)
LFQARmodel <- readRDS("LFQARmodel.rds")
LFQARmodel_etc <- readRDS("ARmodel_list.rds")
iters <- 300
...
max <- list()
for (i in 1:iters){
  max[[i]] <- max(LFQARmodel_etc$inds[[i]][[1]][,2])
  if (max[[i]] > LFQARmodel1[[i]]$Linf){
    max[[i]] = LFQARmodel1[[i]]$Linf
  }
}

res_cc <- readRDS("AR_res_cc.rds")
for(i in 1:iters){
...
for(i in 1:iters){
  AR_res_bheq[[i]] <- bheq(LFQARmodel_etc$inds[[i]][[1]][,2], type = 2, K = LFQARmodel1[[i]]$K, Linf = LFQARmodel1[[i]]$Linf, Lc = LFQARmodel1[[i]]$L50, La = max[[i]], nboot = 500)
}

saveRDS(AR_res_bheq, file = "AR_res_bheq.rds")


#' Extract inputs
list <- list()
for(i in 1:iters){
  list[[i]] <- LFQARmodel_etc$inds[[i]]
}
...
for (i in 1:iters){
  ...
  W_a[[i]] <- lwa*L_a[[i]]^lwb
  Mat_a[[i]] <- as.numeric(sum(list[[i]][[1]][,4] > LFQARmodel1[[i]]$Lmat))
  M[[i]] <- LFQARmodel1[[i]]$M
}

for(i in 1:iters){
  F[[i]] <- AR_res_bheq[[i]]$z - M[[i]]
  if(F[[i]] < 0){
    F[[i]] = 0
  }
}
...
#' run model
AR_res_SB <- list(SPR = rep(NA, 100), YPR = rep(NA, 100))
for (i in 1:iters){
  AR_res_SB$SPR[[i]] <- SPR_SB(ages[[i]], Mat_a[[i]], W_a[[i]], M[[i]], F[[i]])
  AR_res_SB$YPR[[i]] <- YPR_SB(F[[i]], ages[[i]], M[[i]], R0, W_a[[i]])
}

#' save SB
saveRDS(AR_res_SB, file = "AR_res_SB.rds")
...
saveRDS(LBSPR_outs, file = "AR_res_LBSPR.rds")
...
for (i in 201:300){
  AR_res_LIME[[i]] <- run_LIME(modpath = NULL,
                                   lh = lh[[i]],
...
p <- list()
for (i in 1:iters){
  p[[i]] <- AR_res_LIME[[i]]$Report$SPR_t
}


saveRDS(AR_res_LIME, file = "AR_res_LIME.rds")
