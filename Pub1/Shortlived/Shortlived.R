...
# modpath = "D:/DPLBM/Pub1/Shortlived"
modpath = getwd()
iteration = 1:300

#' Run Model
set.seed(1)
sigSu <- virtualPop_par(Linf.mu = Linf.mu, 
                    Linf.cv = Linf.cv,
                    K.mu = K.mu,
                    K.cv = K.cv,
                    ts = ts,
...
saveRDS(Basemodel, file = "LFQshortmodel.rds")
...
saveRDS(Basemodel_list, file = "Shortlist.rds")
