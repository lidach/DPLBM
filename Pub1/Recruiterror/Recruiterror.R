...
errormodel_ls <- list.files(path = modpath, recursive = T, pattern = "True.rds")
errormodel <- lapply(errormodel_ls, function(x) {
  dir = readRDS(file = x)
  errormodel = dir$lfqbin
})

diri <- list()
for (i in 1:iters){
  diri[[i]] <- readRDS(file = errormodel_ls[i])
  errormodel[[i]]$catch <- errormodel[[i]]$catch[,which(format(errormodel[[i]]$dates, "%Y") %in% "2014")]
  errormodel[[i]]$dates <- errormodel[[i]]$dates[format(errormodel[[i]]$dates, "%Y") %in% "2014"]
  class(errormodel[[i]]) <- "lfq"
  errormodel[[i]]$par <- list(Linf = Linf.mu,
                            K = K.mu)
}
...
saveRDS(errormodel, file = "LFQerrormodel.rds")
...
#' save all
errormodel_list <- list(ref = ref, inds = inds)
saveRDS(errormodel_list, file = "Errorlist.rds")


unlink(errormodel_ls)
rm(list = ls())
