lfqModifydev <- function (lfq, par = NULL, bin_size = NA, vectorise_catch = FALSE, 
          plus_group = FALSE, lfq2 = NA) 
{
  if (class(lfq) != "lfq") 
  ...
  if (!any(is.na(lfq2))) {
    ...
    mergi <- merge(data.frame(dates = dates, x = dates), 
                   ...
                   all = TRUE)
    mergi2 <- merge(data.frame(midLengths = midLengths, 
                               x = midLengths), data.frame(midLengths = midLengths2, 
                                                           y = midLengths2), by = "midLengths", all = TRUE)
    indY <- which(is.na(mergi2$y) & mergi2$midLengths > 
                    max(midLengths2))
    ...
    for (i in 1:dim(designMat)[2]) {
      ...
      temp2 <- data.frame(midLengths = mergi2$midLengths, 
                          catch2 = catch2[, i])
      temp3 <- merge(temp1, temp2, by = "midLengths", 
                     all = TRUE)
  ...
  if (!is.na(bin_size)) {
    ...
    for (i in 1:length(unique(dates))) {
      ...
      if (length(unique(dates)) > 1) {
        lengthi <- rep.int(midLengths, times = as.numeric(catch[, 
                                                                dates == sampli]))
      }
      else {
        lengthi <- rep.int(midLengths, times = as.numeric(catch[dates == 
                                                                  sampli]))
      }
      cuti <- cut(lengthi, breaks = bin.breaks, labels = midLengthsNEW, 
                  include.lowest = TRUE)
      freq <- plyr::count(cuti)
      colnames(freq) <- c("midLengths", dati)
      listi[[i]] <- merge(LF_dat, freq, by.x = "midLengths", 
                          all.x = TRUE)[, 2]
    }
  ...
  if (isTRUE(plus_group) | is.numeric(plus_group)) {
    if (isTRUE(plus_group)) {
      ...
      pg = -1
      if(max(midLengths) > lfq$Linf){
      pg <- midLengths[midLengths < lfq$Linf]
      pg <- max(pg)
      }else{
        pg <- max(midLengths)
      }
      ...
      if (which(midLengths == pg) < (length(catch) - 1)) {
        addplus <- sum(catch[((which(midLengths == pg) + 
                                 1):length(catch))])
                                           ...
                                           1)) {
        addplus <- catch[(which(midLengths == pg) + 
                            1)]
      }
      else if (which(midLengths == pg) == (length(catch))) {
        addplus <- 0
  ...
  res <- list(dates = dates, midLengths = midLengths, catch = catches)
  if (!is.null(par)) {
    res$Linf <- par$Linf
    res$K <- par$K
  ...
  if ("t0" %in% names(lfq)) 
    res$t0 <- lfq$t0
  if ("t_anchor" %in% names(lfq)) 
    res$t_anchor <- lfq$t_anchor
  if ("C" %in% names(lfq)) 
    res$C <- lfq$C
  if ("ts" %in% names(lfq)) 
    res$ts <- lfq$ts
  ...
  if ("E" %in% names(lfq)) 
    res$E <- lfq$E
  if ("FM_calc" %in% names(lfq)) 
    res$FM_calc <- lfq$FM_calc
  if ("a" %in% names(lfq)) 
    res$a <- lfq$a
  if ("b" %in% names(lfq)) 
    res$b <- lfq$b
  ...
  if ("L95" %in% names(lfq)) 
    res$L95 <- lfq$L95
  if ("s_list" %in% names(lfq)) 
    res$s_list <- lfq$s_list
  if (class(res) != "lfq") {
    class(res) <- "lfq"
  }
  return(res)
