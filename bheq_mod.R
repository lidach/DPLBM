bheq_LBRA <- function(len = NULL, type = c(1, 2), K = NULL, Linf = NULL, 
          Lc = NULL, La = NULL, t0 = NULL, nboot = 100) 
{
  if (is.null(len)) 
  ...
  if (any(type == 2) & is.null(La)) 
    stop("La is required for method 2")
  len <- len[!is.na(len)]
  lengths <- len[len >= Lc]
  rown <- length(type)
  results <- data.frame(method = as.character(rep("NA", rown)), 
                        meanlen = as.numeric(rep(NA, rown)), n = rep(as.numeric(NA), 
                                                                     rown), z = rep(as.numeric(NA), rown), SE = rep(as.numeric(NA), 
                                                                                                                    rown), stringsAsFactors = FALSE)
  cnt = 0
  if (any(type == 1)) {
    mean.boot1 <- function(x, i) {
      f <- function(x) {
        (K * (Linf - (sum(x)/length(x))))/((sum(x)/length(x)) - 
                                             Lc)
      }
      f(lengths[i])
    }
    ...
    cnt <- cnt + 1
    results[cnt, 1] <- "BH"
    results[cnt, 2:5] <- c(mL, length(lengths), dd[[1]], 
                           apply(dd$t, 2, sd))
  }
  if (any(type == 2)) {
    mean.boot1 <- function(x, i) {
